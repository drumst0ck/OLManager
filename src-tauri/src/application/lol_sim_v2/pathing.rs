...
pub(super) fn choose_lane_anchor_index(
    ...
    // 2. Dónde está el frente enemigo (nuestra amenaza)
    let enemy_front = minions
        .iter()
        .filter(|m| {
            m.alive
                && normalized_team(&m.team) != normalized_team(&champion.team)
        ...
        .min_by(|a, b| a.path_index.cmp(&b.path_index));
    ...
    // El punto medio "ideal" para esperar, restringido por el avance enemigo
    let safe_mid_point = mid_lane_index.min(safety_limit_idx);

    // La decisión final:
    // Avanzamos hasta nuestra oleada, o hasta el medio seguro, ¡el que esté más adelante!
    let target_idx = front_idx.max(safe_mid_point);

    target_idx.clamp(1, lane_last_idx)
}
